...
- [ ] **7.1: Update Finance Functions**
  ...
  - **Verified**: 10 functions with 15+ direct queries found (Lines 169, 189, 249, 340, 412, 484, 498, 569, 583, 637, 657, 700, 775, 808, 850)

- [ ] **7.2: Update Search Service** ⚠️ **NEW - DISCOVERED IN DEPENDENCY ANALYSIS**
  - **File**: `src/lib/services/search.ts`
  - **Lines**: 59 (findMany), 76 (count)
  - Add `isDuplicate: false` filter to semantic search queries
...
- [ ] **7.4: Update Re-categorize Endpoint** ⚠️ **NEW - DISCOVERED IN DEPENDENCY ANALYSIS**
  - **File**: `src/app/api/receipts/re-categorize/route.ts`
  - **Lines**: 34 (findMany), 150 (groupBy)
  - Add `isDuplicate: false` filter to batch queries
  - Impact: MEDIUM - affects batch re-categorization feature
  - **Priority**: 🟡 HIGH

- [ ] **7.5: Update AI Finance Agent**
  ...
  - **Note**: No direct queries found - uses financeFunctions.ts (will be updated in 7.1)

**Estimated Time:** 4-5 hours (updated to reflect additional files)
...
### Services
10. `src/lib/services/financeFunctions.ts` - Update queries to exclude duplicates (10 functions, 15+ queries)
11. `src/lib/services/export.ts` - Add duplicate handling to export (Line 101)
12. `src/lib/services/financeAgent.ts` - Update agent queries (no direct queries, uses financeFunctions.ts)
13. `src/lib/services/search.ts` - ⚠️ **NEW** - Update semantic search to exclude duplicates (Lines 59, 76)
14. `src/app/api/receipts/re-categorize/route.ts` - ⚠️ **NEW** - Update batch re-categorization (Lines 34, 150)

### Frontend Components
15. `src/components/receipts/ReceiptList.tsx` - Show duplicate indicators
16. `src/components/dashboard/ReceiptStats.tsx` - Already receives filtered stats (minimal changes)
17. `src/components/receipts/ReceiptViewerModal.tsx` - Show duplicate status and links
18. `src/components/dashboard/RecentReceipts.tsx` - Handle duplicate display

### Types & Utilities
19. `src/types/receipt.ts` - ⚠️ **NOTE**: Does not exist (uses Prisma-generated types)
20. `src/lib/utils.ts` - Add any duplicate-related utility functions

---

## ⚠️ UPDATES FROM DEPENDENCY ANALYSIS (2025-12-02)

**Two additional critical files discovered during systematic dependency mapping**:

1. **src/lib/services/search.ts** (Lines 59, 76)
   - Impact: HIGH - Semantic search functionality
   - Queries: `prisma.receipt.findMany()`, `prisma.receipt.count()`
   - Action: Add `isDuplicate: false` filter to both queries
   - Added to Phase 7.2

2. **src/app/api/receipts/re-categorize/route.ts** (Lines 34, 150)
   - Impact: MEDIUM - Batch re-categorization feature
   - Queries: `prisma.receipt.findMany()`, `prisma.receipt.groupBy()`
   - Action: Add `isDuplicate: false` filter to queries
   - Added to Phase 7.4

**Total Files to Modify**: 20 files (increased from 19)
**Total Direct Queries to Update**: ~30 queries (increased from ~20 estimated)
